...
(function (root) {
  ...
  class Position {
    constructor() {
      ...
      this.npm1 = 0;
      this.pieces = [];
      this.wpCount = [0, 0, 0, 0, 0, 0, 0, 0];
      this.bpCount = [0, 0, 0, 0, 0, 0, 0, 0];
      this.wpFile = [-1, -1, -1, -1, -1, -1, -1, -1];
      this.bpFile = [8, 8, 8, 8, 8, 8, 8, 8];
      this.wBishops = 0;
      this.bBishops = 0;
    }
  }

  function rebuildPawnFiles(pos) {
    pos.wpCount.fill(0); pos.bpCount.fill(0);
    pos.wpFile.fill(-1); pos.bpFile.fill(8);
    pos.wBishops = 0; pos.bBishops = 0;
    for (let i = 0; i < pos.pieces.length; i++) {
      const p = pos.pieces[i].p, sq = pos.pieces[i].sq;
      const t = p & 7, c = p >> 3, f = sq & 7, r = sq >> 4;
      if (t === PAWN) {
        if (c === WHITE) { pos.wpCount[f]++; if (r > pos.wpFile[f]) pos.wpFile[f] = r; }
        else { pos.bpCount[f]++; if (r < pos.bpFile[f]) pos.bpFile[f] = r; }
      } else if (t === BISHOP) {
        if (c === WHITE) pos.wBishops++; else pos.bBishops++;
      }
    }
  }
  ...
  function makeMove(pos, m) {
    ...
    pos.hist.push({
      ...
      wpC: pos.wpCount.slice(), bpC: pos.bpCount.slice(),
      wpF: pos.wpFile.slice(), bpF: pos.bpFile.slice(),
      wB: pos.wBishops, bB: pos.bBishops
    });
    zXor(pos, pc, from);
